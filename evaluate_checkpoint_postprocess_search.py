#!/usr/bin/env python3
"""
Grid search post-processing parameters for checkpoint evaluation.
Usage:
  python evaluate_checkpoint_postprocess_search.py --model ./output/sleep_stage_cnn.pt --stages 3
"""
import sys
import os
...
def main():
    parser = argparse.ArgumentParser(description="Grid search post-processing parameters")
    ...
    methods = parse_str_list(args.methods, ["majority", "prob_avg", "none"])
    windows = parse_int_list(args.windows, [3, 5, 7])
    min_runs = parse_int_list(args.min_runs, [1, 2, 3])
    ...
    raw_metrics = compute_metrics(test_labels, predictions)
    print("\nRaw metrics")
    ...
    for method in methods:
        if method == "none":
            for min_run in min_runs:
                combos.append((method, 1, min_run))
        else:
            for window in windows_odd:
                for min_run in min_runs:
                    combos.append((method, window, min_run))

    results = []
    for method, window, min_run in combos:
